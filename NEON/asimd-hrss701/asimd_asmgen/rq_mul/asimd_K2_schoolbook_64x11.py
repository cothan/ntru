...
def K2_schoolbook_64x11_8(r_mem, a_mem, b_mem, r_off=0, a_off=0, b_off=0, additive=False):
    for i in range(6):
        p("vld1q_s16 ({}+{}) = y{}".format(16*(i + a_off), a_mem, i)) # move aligned, LOAD
        p("vld1q_s16 ({}+{}) = y{}".format(16*(i + b_off), b_mem, i+6)) # move aligned, LOAD

        pp("vld1q_s16 ({}+{}) = y{}".format(16*(i + a_off) + 8, a_mem, 16 + i)) # move aligned, LOAD
        pp("vld1q_s16 ({}+{}) = y{}".format(16*(i + b_off) + 8, b_mem, 16 + i+6)) # move aligned, LOAD

        if additive:
            p("vaddq_s16 (vld1q_s16({}+{}), y{}) = y{}".format(16*(i + a_off + 11), a_mem, i, i)) # add packed integer
            p("vaddq_s16 (vld1q_s16({}+{}), y{}) = y{}".format(16*(i + b_off + 11), b_mem, i+6, i+6)) # add packed integer

            pp("vaddq_s16 (vld1q_s16({}+{}), y{}) = y{}".format(16*(i + a_off + 11) + 8, a_mem, 16 + i,   16 + i)) # add packed integer
            pp("vaddq_s16 (vld1q_s16({}+{}), y{}) = y{}".format(16*(i + b_off + 11) + 8, b_mem, 16 + i+6, 16 + i+6)) # add packed integer
        ...
        for j in range(min(i+1, 6)):
            if i - j < 6:
                if first:
                    ...
                    pp("vmulq_s16 (y{}, y{}) = y{}".format(16 + j, 16 + 6+ i-j, 16 + 12 + (i % 2))) 
                    ...
                    pp("vmlaq_s16 (y{}, y{}, y{}) = y{}".format(16 + 12 + (i % 2), 16 + j, 16 + 6+ i-j, 16 + 12 + (i % 2))) 
                    # p("vpmullw y{}, y{}, y{}".format(j, 6+ i-j, 15)) 
                    # p("vpaddw y{}, y{}, y{}".format(12 + (i % 2), 15, 12 + (i % 2)))
        p("vst1q_s16 ({}+{}, y{});".format( 16*(i + r_off), r_mem, 12 + (i % 2))) #move aligned, STORE

        pp("vst1q_s16 ({}+{}, y{});".format( 16*(i + r_off) + 8, r_mem, 16 + 12 + (i % 2))) #move aligned, STORE

    for i in range(5):
        p("vld1q_s16 ({}+{}) = y{}".format(16*(6+i + a_off), a_mem, i)) # move aligned, LOAD
        p("vld1q_s16 ({}+{}) = y{}".format(16*(6+i + b_off), b_mem, i+6)) # move aligned, LOAD

        pp("vld1q_s16 ({}+{}) = y{}".format(16*(6+i + a_off) + 8, a_mem, 16 + i)) # move aligned, LOAD
        pp("vld1q_s16 ({}+{}) = y{}".format(16*(6+i + b_off) + 8, b_mem, 16 + i+6)) # move aligned, LOAD

        if additive:
            p("vaddq_s16 (vld1q_s16({}+{}), y{}) = y{}".format(16*(6+i + a_off + 11), a_mem, i, i))
            p("vaddq_s16 (vld1q_s16({}+{}), y{}) = y{}".format(16*(6+i + b_off + 11), b_mem, i+6, i+6))

            pp("vaddq_s16 (vld1q_s16({}+{}), y{}) = y{}".format(16*(6+i + a_off + 11) + 8, a_mem, 16 + i,   16 + i))
            pp("vaddq_s16 (vld1q_s16({}+{}), y{}) = y{}".format(16*(6+i + b_off + 11) + 8, b_mem, 16 + i+6, 16 + i+6))
    ...
    for i in range(9):
        ...
        # p("vmovdqa y{}, {}({})".format(12 + (i % 2), 16*(12+i + r_off), r_mem)) # move aligned, STORE
        p("vst1q_s16 ({}+{}, y{});".format( 16*(12+i + r_off), r_mem, 12 + (i % 2))) # move aligned, STORE
        pp("vst1q_s16 ({}+{}, y{});".format( 16*(12+i + r_off) + 8, r_mem, 16 + 12 + (i % 2))) # move aligned, STORE
    for i in range(5):  # i == 5 is still in place as a[5] resp. b[5]
        p("vld1q_s16 ({}+{}) = y{}".format(16*(i + a_off), a_mem, 12)) #ADD mem, ymm
        p("vaddq_s16 (y{}, y{}) = y{}".format(12,  i, i)) #ADD mem, ymm

        p("vld1q_s16 ({}+{}) = y{}".format(16*(i + a_off), b_mem, 12)) #ADD mem, ymm
        p("vaddq_s16 (y{}, y{}) = y{}".format(12, i+6, i+6)) #ADD mem, ymm

        pp("vld1q_s16 ({}+{}) = y{}".format(16*(i + a_off) + 8, a_mem, 16 + 12)) #ADD mem, ymm
        pp("vaddq_s16 (y{}, y{}) = y{}".format(12 + 16, 16 + i, 16 + i)) #ADD mem, ymm

        pp("vld1q_s16 ({}+{}) = y{}".format(16*(i + a_off) + 8, b_mem, 16 + 12)) #ADD mem, ymm
        pp("vaddq_s16 (y{}, y{}) = y{}".format(12 + 16, 16 + i+6, 16 + i+6)) #ADD mem, ymm
        ...
        if additive:
            ...
            pp("vaddq_s16 (vld1q_s16({}+{}), y{}) = y{}".format(16*(i + a_off + 11) + 8, a_mem, 16 + i,   16 + i)) # ADD mem, ymm
            pp("vaddq_s16 (vld1q_s16({}+{}), y{}) = y{}".format(16*(i + b_off + 11) + 8, b_mem, 16 + i+6, 16 + i+6)) # ADD mem, ymm
    ...
    for j in range(6):
        if j == 0:
            ...
            pp("vmulq_s16 (y{}, y{}) = y{}".format(j + 16 , 16 + 6+ i-j, 16 + target)) 
        else:
            # MUL LOW and ADD
            p("vmlaq_s16 (y{}, y{}, y{}) = y{}".format(target, j, 6+ i-j, target)) 

            pp("vmlaq_s16 (y{}, y{}, y{}) = y{}".format(16 + target, 16 + j, 16 + 6+ i-j, 16 + target)) 
            # p("vpmullw y{}, y{}, y{}".format(j, 6+ i-j, free))   
            # p("vpaddw y{}, y{}, y{}".format(free, target, target))
    # weird centerpiece, deal with this straight away to free up a register
    ...
    pp("vsubq_s16 (y{}, vld1q_s16({}+{})) = y{}".format(target + 16, 8 + 16*(5 + r_off), r_mem,   16 + target))
    pp("vsubq_s16 (y{}, vld1q_s16({}+{})) = y{}".format(target + 16, 8 + 16*(17 + r_off), r_mem,  16 + target))
    ...
    for j in range(1, 5):  # note again that we do not compute [10]
        ...
        pp("vmulq_s16 (y{}, y{}) = y{}".format(16 + 5, 16 + 6+ j, 16 + 12+j-1))
    # this frees up register y5 which held a[5]
    free = 5
    # finish up [6] to [9] (in registers 12 to 15)
    for i in range(6, 10):
        ...
        for j in range(min(i+1, 6)):
            ...
            if i - j < 6:
                ...
                pp("vmlaq_s16 (y{}, y{}, y{}) = y{}".format(target + 16 , 16 + j, 16 + 6+ i-j, 16 + target)) 
                # p("vpmullw y{}, y{}, y{}".format(j, 6+ i-j, free))
                # p("vpaddw y{}, y{}, y{}".format(free, target, target))
        ...
        for j in range(min(i+1,6)):
            if i - j < 6:
                if first:
                    ...
                    pp("vmulq_s16 (y{}, y{}) = y{}".format(16 + j, 16 + 6+ i-j, 16 + target))
                    first = False
                else:
                    # MUL LOW and ADD
                    p("vmlaq_s16 (y{}, y{}, y{}) = y{}".format(target, j, 6+ i-j, target)) 
    ...
    for i in range(5):
        ...
        pp("vld1q_s16 ({}+{}) = y{}".format(16*(6+i + r_off) + 8, r_mem, 16 + i)) # LOAD mem to ymm

        # p("vpsubw {}({}), y{}, y{}".format(16*(12+i + r_off), r_mem, i, i)) # SUB mem, ymm
        p("vsubq_s16 (y{}, vld1q_s16({}+{}) ) = y{}".format(i, 16*(12+i + r_off), r_mem, i)) # SUB mem, ymm

        pp("vsubq_s16 (y{}, vld1q_s16({}+{}) ) = y{}".format(i + 16, 16*(12+i + r_off) + 8, r_mem, 16 +i)) # SUB mem, ymm
        if i < 4:
            p("vsubq_s16 (y{}, y{}) = y{}".format(t2[6 + i], i, i+6)) # SUB ymm, ymm

            pp("vsubq_s16 (y{}, y{}) = y{}".format(t22[6 + i], 16 + i, 16 + i+6)) # SUB ymm, ymm
            if i < 3:
                p("vsubq_s16 ( y{}, vld1q_s16({}+{})) = y{}".format( i+6, 16*(18+i + r_off), r_mem, i+6)) # SUB ymm, ymm

                pp("vsubq_s16 ( y{}, vld1q_s16({}+{})) = y{}".format( 16 + i+6, 8 + 16*(18+i + r_off), r_mem, 16 + i+6)) # SUB ymm, ymm
            p("vst1q_s16 ({}+{}, y{});".format( 16*(12+i + r_off), r_mem, i+6)) # STORE ymm to mem

            pp("vst1q_s16 ({}+{}, y{});".format( 16*(12+i + r_off) + 8, r_mem, 16 + i+6)) # STORE ymm to mem
        # ADD, SUB and STORE
        p("vaddq_s16 (y{}, y{}) = y{}".format(t2[i], i, i))

        pp("vaddq_s16 (y{}, y{}) = y{}".format(t22[i], 16 + i, 16 + i))

        p("vsubq_s16 (y{}, vld1q_s16({}+{})) = y{}".format(i, 16*(i + r_off), r_mem, i))

        pp("vsubq_s16 (y{}, vld1q_s16({}+{})) = y{}".format(16 + i, 16*(i + r_off) + 8, r_mem, 16 + i))
