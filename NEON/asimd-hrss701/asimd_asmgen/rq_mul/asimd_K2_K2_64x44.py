...
def done(t0, t1, t2):
    ...
    for i in range(22):
        ...
        for j in range(2):
            ...
            p("y{} = vld1q_u16({} + {});".format(t2[j], 16*(i + b_off) + slide, b_mem))
            p("y{} = vld1q_u16({} + {});".format(t3[j], 16*(22 + i + b_off) + slide, b_mem))
            p("y{} = vaddq_u16(y{}, y{});",format(t2[j], t2[j], t3[j]))
            p("vst1q_u16({} + rsp, y{});".format(16*(b_in_rsp + i) + slide, t2[j]))
    ...
    for i in range(10):
        ...
        for j in range(2):
            ...
            # t3 save for later 
            p("y{} = vsubq_u16(y{} ,y{});".format(t3[j], t1[j], 16 + i + j))
            save.append(16 + i + j)
...
def K2_K2_transpose_64x44(r_real='%rdi', a_real='%rsi', b_real='%rdx', coeffs=44, transpose=True):
    ...
    p("uint16_t rsp[{}];".format((44 + 44 + 96 + 22 + 22 + 22 + 44) * 16))
